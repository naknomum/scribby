...
//console.log('%d (%d,%d) %s', ev.keyCode, this._keyX, this._keyY, this._keyText);
    ...
    static dragbarClick(buttonEl) {
        ...
        if (buttonEl.classList.contains('dragbar-reset')) {
            ...
            var dl = document.createElement('a');
            dl.href = this.makeDataUrl('image/svg+xml', '<!-- notey ' + note.version + ' https://github.com/naknomum/scribby created ' + new Date().toISOString() + ' at ' + document.location.href + ' id=' + note.id + ' -->' + note.svg.outerHTML);
